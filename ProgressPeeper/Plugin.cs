...
public sealed partial class Plugin : IDalamudPlugin
{
    ...
    public Plugin(IDalamudPluginInterface pluginInterface)
    {
        ...
        pluginInterface.UiBuilder.OpenMainUi   += windowSystem.Draw;
        ...
        Configuration.OnSave += (sender, args) =>
                                {
                                    if (args.UseShortCommand)
                                    {
                                        ...
                                        Services.CommandManager.AddHandler(
                                            ...
                                            new CommandInfo(OnCommand)
                                            {
                                                HelpMessage = """Short-hand for /ppeep.""",
                                            }
                                        );
                                    }
                                    else
        ...
        (partyMemberListener = new PartyMemberListener()).OnJoin += (sender, args) =>
                                                                    {
    ...
    private void PrintChatError(string msg)
    {
        var builder = new SeStringBuilder();

        if (Configuration.PrintLogo)
        {
            ...
            async () =>
            {
                ...
                try
                {
                    ...
                    foreach (var progressionTarget in progressionTargets)
                    {
                        ...
                        FancyChat.WithLink(
                            ref builder,
                            $"https://tomestone.gg{progressionTarget.Link}",
                            (ref builder) =>
                            {
                                FancyChat.WithEncounterStyle(
                                    ref builder,
                                    2,
                                    progressionTarget.Name,
                                    (ref builder) =>
                                    {
                    ...
                    foreach (var clearedEncounter in clearedEncounters)
                    {
                        ...
                        FancyChat.WithLink(
                            ref builder,
                            clearedEncounter.Link(apiCharacter.Name, apiCharacter.Id),
                            (ref builder) =>
                            {
                                ...
                                FancyChat.WithEncounterStyle(
                                    ref builder,
                                    2,
                                    clearedEncounter.CompactName,
                                    (ref builder) => { builder.AddText(clearedEncounter.CompactName); }
                                );
                            }
                        );
                    }
                ...
                catch (Exception e)
                {
                    ...
                    builder.AddUiForeground(
                        " Unable to fetch data from Tomestone" +
                        e switch
                        {
                            null => ".",
