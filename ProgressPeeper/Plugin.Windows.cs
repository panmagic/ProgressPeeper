...
public sealed partial class Plugin
{
    private class ConfigurationWindow : Window, IDisposable
    {
        ...
        public override void Draw()
        {
            ...
            var printLogo                   = configuration.PrintLogo;
            ...
            if (ImGui.CollapsingHeader("Instructions", ImGuiTreeNodeFlags.Selected))
            {
                ...
                if (ImGui.Button("Go to your Tomestone account settings."))
                    Util.OpenLink("https://tomestone.gg/profile/account/");

                ImGui.AlignTextToFramePadding();
                ImGui.BulletText("Create an API Access Token.");

                var image = Services.TextureProvider
                                    .GetFromFile(Path.Combine(Services.PluginInterface.AssemblyLocation.Directory?.FullName!, "guide-1.png"))
                                    .GetWrapOrDefault();
            ...
            if (ImGui.CollapsingHeader("Extra Options"))
            {
                ...
                if (ImGui.Checkbox($"{Pad}Print Logo{Pad}", ref printLogo))
                {
                    configuration.PrintLogo = printLogo;
                    configuration.Save();
                }
